...
function wordCounter() {
  inquirer
    ...
    .then((answers) => {
      ...
      console.log(
        chalk.blue(
          `\nYour input "${chalk.greenBright(input)}" contains ${chalk.yellowBright(wordCount)} ${
            chalk.magenta(wordCount === 1 ? "word" : "words"
         )} and ${chalk.greenBright(characterCount)} characters.`
        )
      );
      ...
      if (characterCount >= 10) {
        console.log(
          chalk.yellow(
            `\t Your input is ${chalk.greenBright(characterCount)} characters. \n`
          )
        );
      } else {
        ...
        .prompt([
          {
            ...
            message: chalk.yellowBright(
              " Do you want to count another sentence or word?"
            ),
          },
        ])
        .then((answer) => {
